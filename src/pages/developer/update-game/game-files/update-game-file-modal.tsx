...
const UpdateGameFileModal = ({
  ...
  const displayNameValidator = async (_: any, value: string) => {
    ...
    const originalExt = file.displayName.split(".").pop()?.toLowerCase();
    const newExt = value.split(".").pop()?.toLowerCase();

    if (originalExt !== newExt) {
      return Promise.reject(
        new Error(`The file extension must remain ".${originalExt}"`)
      );
    }
  ...
  const handleSubmitLogic = async (values: FieldType) => {
    ...
    const result = await updateGameFile(file.id, {
        ...
        <Form.Item label="Current File">
          <div>
            ...
            <span style={{ color: "#888" }}>
              ({file ? formatMegabytes(file.size) : "Unknown size"})
            </span>
          </div>
        </Form.Item>
